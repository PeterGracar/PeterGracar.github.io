...
simulations/                All interactive simulation HTML lives here, and is
  ...
  levy-vs-bm-3d-standalone.html  Standalone 3D variant of the Lévy-vs-Brownian
                                    simulation: free-space random walks in R³
                                    whose step *directions* are sampled
                                    uniformly on the unit sphere (the walks are
                                    not confined to a sphere surface), drawn
                                    with Three.js (WebGL);
